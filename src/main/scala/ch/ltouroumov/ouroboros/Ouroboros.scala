...
object Ouroboros {
  ...
  Registry.BLOCKS.register(MinecraftForge.EVENT_BUS)
  Registry.ITEMS.register(MinecraftForge.EVENT_BUS)
  ...
  private def enqueueIMC(event: InterModEnqueueEvent): Unit = {
    ...
    InterModComms.sendTo("ouroboros", "helloworld", () => {
...
object Registry {
  ...
  val BLOCKS: DeferredRegister[Block] = DeferredRegister.create(ForgeRegistries.BLOCKS, Ouroboros.MOD_ID)
  val ITEMS: DeferredRegister[Item] = DeferredRegister.create(ForgeRegistries.ITEMS, Ouroboros.MOD_ID)
